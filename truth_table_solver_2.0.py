...
# submit button flag variable
submit1_pressed = False
submit2_pressed = False
...
def get_inputs():
    global truth_table_input, truth_table_symbols, button_values, submit1_pressed, submit2_pressed
    ...
    if (input_value.isnumeric() and 2 <= int(input_value) <= truth_table_input_max) and (symbols_value.isalpha() == 1) and (len(set(symbols_value)) == len(symbols_value)) and (len(symbols_value) == int(input_value)):
        ...
        submit1_pressed = True  # Set the flag to indicate button has been pressed
...
def create_toggle_buttons(num_buttons):
    ...
    # display truth table output buttons
    buttons = []
    ...
    output_submit_button = Button(root, text="Submit", padx=10, pady=5, command=button2_pressed)
    output_submit_button.grid(row=4, column=2, sticky="nsew")
    #
    # # Show Submit Button
    # BUTTON = Button(root, text=f"Submit", command=get_inputs, padx=10, pady=5)
...
def get_simplified_expression(input_symbols, tt_inputs, tt_outputs):
    ...
    if ("x" in tt_outputs) or ("X" in tt_outputs):
        ...
        # simplified equivalent dont care
        simplified_label1 = Label(content_frame2, text="Simplified Boolean Expression dont care: ")
        ...
        simplified_dont_care_output = "Z = " + simplified_expression_dont_care
        split_simplified_dont_care_output = split_string(simplified_dont_care_output)
        simplified_label2 = Label(content_frame2, text=split_simplified_dont_care_output, width=59, padx=10, pady=10, relief="solid")
        simplified_label2.grid(row=1, column=0, sticky="we")
        simplified_label2.configure(font=("Arial", font_size, "bold"), foreground="black")

        # copy button
        simplified_copy_button = Button(content_frame2, text=f"copy", command=lambda: copy_to_clipboard(simplified_dont_care_output), padx=10, pady=5)
        ...
        simplified_label2 = Label(content_frame2, text=split_simplified_output, width=59, padx=10, pady=10, relief="solid")
        simplified_label2.grid(row=1, column=0, sticky="we")
        simplified_label2.configure(font=("Arial", font_size, "bold"), foreground="black")

        # copy button
        simplified_copy_button = Button(content_frame2, text=f"copy", command=lambda: copy_to_clipboard(simplified_output), padx=10, pady=5)
        simplified_copy_button.grid(row=2, column=0, sticky="e")
...
def get_verilog_output(expression_str):
    ...
    verilog_label2 = Label(content_frame2, text=split_verilog_output, width=59, padx=10, pady=10, relief="solid")
    verilog_label2.grid(row=4, column=0, sticky="we")
    verilog_label2.configure(font=("Arial", font_size, "bold"), foreground="black")

    # copy button
    verilog_copy_button = Button(content_frame2, text=f"copy", command=lambda: copy_to_clipboard(verilog_output), padx=10, pady=5)
...
def get_vhdl_output(expression_str):
    ...
    vhdl_label2 = Label(content_frame2, text=split_vhdl_output, width=59, padx=10, pady=10, relief="solid")
    vhdl_label2.grid(row=7, column=0, sticky="we")
    vhdl_label2.configure(font=("Arial", font_size, "bold"), foreground="black")

    # copy button
    vhdl_copy_button = Button(content_frame2, text=f"copy", command=lambda: copy_to_clipboard("Z <= " + expression_str + ";"), padx=10, pady=5)
